""" Contains a class that represents tinychat.com privacy page for a tinychat account (room). """
...
class TinychatPrivacyPage:
    ...
    def _is_tc_account(account_name):
        ...
        response = utilities.web.http_get(url=url, json=True)
    ...
    def clear_bans(self):
        """ Clear all room bans. """
        url = 'https://tinychat.com/ajax/user/clearbans'
        header = {
            'X-Requested-With': 'XMLHttpRequest',
            'Referer': self._privacy_url
        }
        response = utilities.web.http_post(url, post_data={}, header=header, json=True, proxy=self._proxy)
        if response['json']['room'] is not None:
            return True
        return False
    ...
    def parse_privacy_settings(self, response=None):
        ...
        if response is None:
            response = utilities.web.http_get(self._privacy_url, header=header, proxy=self._proxy)
    ...
    def _update(self):
        ...
        response = utilities.web.http_post(self._privacy_url, post_data=form_data, header=header, proxy=self._proxy)
        self.parse_privacy_settings(response=response)
        self._privacy_url = 'https://tinychat.com/settings/privacy?saved=1'
